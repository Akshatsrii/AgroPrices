...
import { UserPlus, ArrowRight, MapPin, Tractor } from 'lucide-react';

const clerkPubKey = import.meta.env.VITE_CLERK_PUBLISHABLE_KEY;

export function SignUpPage() {
  const navigate = useNavigate();
  const { sendOtp, updateFarmerProfile } = useAuthStore();
  ...
  return (
    <div className="w-full bg-white/95 backdrop-blur-xl border border-emerald-100 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Register Farmer</h2>
          <p className="text-xs text-gray-500">Create your free AgroPrice AI account</p>
        </div>
      </div>
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-semibold text-gray-900 focus:bg-white focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-16 pr-4 py-3 text-sm font-semibold text-gray-900 focus:bg-white focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
            />
          </div>
        </div>
